<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx.master" AutoEventWireup="true" 
    CodeBehind="UpdateJobLevelPositionRenumerationEntry.aspx.cs" Inherits="CodeX.Muses.Web.Inventory.Program.UpdateJobLevelPositionRenumerationEntry" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        ...
        function onGetJobLevelFilterExpression() {
            var TransactionID = $('#<%=hdnTransactionID.ClientID %>').val();
            var filterExpression = "IsDeleted = 0 AND JobLevelPositionID NOT IN (SELECT JobLevelPositionID FROM TransJobLevelPositionRenumerationDt where TransactionID = " + TransactionID + ")";
            return filterExpression;
        }

        function ontacJobLevelIDSearchClick() {
            openSearchDialog('joblevelposition', onGetJobLevelFilterExpression(), function (value) {
                var filterExpression = onGetJobLevelFilterExpression() + " AND JobLevelPositionID = '" + value + "'";
                Methods.getObject('GetvJobLevelPositionList', filterExpression, function (result) {
                    if (result != null) {
                        tacJobLevelID.setValue(result.JobLevelPositionID);
                        tacJobLevelID.setText(result.JobLevelPositionType);
                    }
                    else {
                        tacJobLevelID.setValue('');
                        tacJobLevelID.setText('');
                                        ...
                                        <td valign="top">
                                            <table style="width: 100%">
                                                ...
                                                <td class="tdLabel"><label class="lblMandatory" id="lblPosition"><%=GetLabel("Level")%></label></td>
                                                 <td>
                                                    <cdx:CodeXAutoCompleteTextBox runat="server" Width="300px" ID="tacJobLevelID" ClientInstanceName="tacJobLevelID" MethodName="GetvJobLevelPositionList" GetFilterExpressionFunction="onGetJobLevelFilterExpression"
                                                        SearchFields="JobLevelPositionType,JobLevelPositionID" TextField="JobLevelPositionType" ValueField="JobLevePositionlID" SearchText="${JobLevelPositionType} (<b>${OrganizationPositionName}</b>)" OrderByExpression="JobLevelPositionID">
                            ...
                            <dx:PanelContent ID="PanelContent1" runat="server">
                                <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto;
                                    ...
                                    <asp:GridView ID="grdView" runat="server" CssClass="tblTransactionEntryResult"
                                        ...
                                        <Columns>
                                            <asp:BoundField DataField="JobLevelPositionType" HeaderText="Golongan" />
                                            <asp:BoundField DataField="OrganizationPositionName" HeaderText="Jabatan" />
                                            <asp:TemplateField HeaderStyle-Width="80px" ItemStyle-HorizontalAlign="Center">
                                                <ItemTemplate>
                                                    <div style='float:right;<%#IsEditable().ToString() == "False" ? "display:none" : "" %>' class="divDetailDelete"></div>
                                                    <input type="hidden" value="<%#Eval("JobLevelPositionID") %>" bindingfield="JobLevelPositionID" />
                                                    <input type="hidden" value="<%#Eval("JobLevelPositionType") %>" bindingfield="JobLevelPositionType" />
