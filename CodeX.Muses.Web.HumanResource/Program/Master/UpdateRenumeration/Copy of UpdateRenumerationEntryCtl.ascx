...
//            $('#</%=chkIsSchedule.ClientID %>').prop('checked', false);
        ...
        setTimeout(function () {
            $('#<%=hdnLstDayTypeID.ClientID %>').val('');
            ddeDayType.SetText('');
            $('.chkDayType input:checked').each(function () {
...
//    }
    ...
    $('.chkDayType input').change(function () {
        setDdeDayTypeText();
    });

    function setDdeDayTypeText() {
        var lstDayTypeID = '';
        var lstDayTypeName = '';
        $('.chkDayType input:checked').each(function () {
            if (lstDayTypeName != '') {
            ...
            lstDayTypeID += $(this).parent().attr('daytypeid');
            lstDayTypeName += $(this).parent().attr('daytypename');
        });
        $('#<%=hdnLstDayTypeID.ClientID%>').val(lstDayTypeID);
        ddeDayType.SetText(lstDayTypeName);
    }
        ...
        <div id="entryDetailContainerPopup" class="entryDetailContainer" style="display: none">
            <fieldset id="fsTrxPopup" style="margin:0"> 
                ...
                <table>
                    ...
                    <tr>
                        <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Tipe Hari")%></label></td>
                        <td colspan="5">
                            <dxe:ASPxDropDownEdit ClientInstanceName="ddeDayType" ID="ddeDayType"
                                Width="300px" runat="server" EnableAnimation="False">
                                <DropDownWindowStyle BackColor="#EDEDED" />
                                <DropDownWindowTemplate>
                                    <asp:Repeater ID="rptDayType" runat="server" OnItemDataBound="rptDayType_ItemDataBound">
                                        <ItemTemplate>
                                            <asp:CheckBox id="chkDayType" CssClass="chkDayType" runat="server"  /> <%#Eval("StandardCodeName")%><br />
                                        </ItemTemplate>
                                    </asp:Repeater>
                                </DropDownWindowTemplate>
