<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx.master" AutoEventWireup="true" 
    CodeBehind="UpdateJobLevelPerformanceIndicatorRenumerationEntry.aspx.cs" Inherits="CodeX.Muses.Web.Inventory.Program.UpdateJobLevelPerformanceIndicatorRenumerationEntry" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        function onLoad() {
            ...
            $('#divTransactionAdd').click(function (evt) {
                if (IsValid(evt, 'fsMPEntry', 'mpEntry')) {
                    editedLineAmount = 0;
                    tacPerformanceIndicatorID.setValue('');
                    tacPerformanceIndicatorID.setText('');
        ...
        //#region WorkYears
        function onGetPerformanceIndicatorFilterExpression() {
            var TransactionID = $('#<%=hdnTransactionID.ClientID %>').val();
            var filterExpression = "IsDeleted = 0 AND JobLevelPerformanceIndicatorID NOT IN (SELECT JobLevelPerformanceIndicatorID  FROM TransJobLevelPerformanceIndicatorRenumerationDt WHERE TransactionID = " + TransactionID + ")";
            //alert(filterExpression);
            return filterExpression;
        }

        function onTacPerformanceIndicatorIDSearchClick() {
            openSearchDialog('joblevelperformanceindicator', onGetPerformanceIndicatorFilterExpression(), function (value) {
                var filterExpression = onGetPerformanceIndicatorFilterExpression() + " AND JobLevelPerformanceIndicatorID = '" + value + "'";
                alert(filterExpression);
                Methods.getObject('GetJobLevelPerformanceIndicatorList', filterExpression, function (result) {
                    if (result != null) {
                        tacPerformanceIndicatorID.setValue(result.PerformanceIndicatorID);
                        tacPerformanceIndicatorID.setText(result.PerformanceIndicatorName);
                    }
                    else {
                        tacPerformanceIndicatorID.setValue('');
                        tacPerformanceIndicatorID.setText('');
        ...
        function onTacPerformanceIndicatorIDValueChanged() {
        }
        //#endregion
    </script>    
                                        ...
                                        <td valign="top">
                                            <table style="width: 100%">
                                                ...
                                                <td class="tdLabel"><label class="lblMandatory" id="lblPosition"><%=GetLabel("Performance")%></label></td>
                                                 <td>
                                                    <cdx:CodeXAutoCompleteTextBox runat="server" Width="300px" ID="tacPerformanceIndicatorID" ClientInstanceName="tacPerformanceIndicatorID" MethodName="GetJobLevelPerformanceIndicatorList" GetFilterExpressionFunction="onGetPerformanceIndicatorlFilterExpression"
                                                        SearchFields="JobLevelPerformanceIndicatorName,JobLevelPerformanceIndicatorID" TextField="JobLevelPerformanceIndicatorName" ValueField="JobLevelPerformanceIndicatorID" SearchText="${PerformanceIndicatorName}  " OrderByExpression="JobLevelPerformanceIndicatorName">
                                                        <ClientSideEvents ButtonSearchClick="function(){ onTacPerformanceIndicatorIDSearchClick(); }"
                                                            ValueChanged="function(){ onTacPerformanceIndicatorIDValueChanged(); }" />
                                                    </cdx:CodeXAutoCompleteTextBox>   
                                                </td>
                                                </tr>
                                            </table>
                                        ...
                                        <Columns>
                                            ...
                                            <asp:TemplateField HeaderStyle-Width="80px" ItemStyle-HorizontalAlign="Center">
                                                <ItemTemplate>
                                                    ...
                                                    <input type="hidden" value="<%#Eval("JobLevelPerformanceIndicatorName") %>" bindingfield="JobLevelPerformanceIndicatorName" />
                                                    <input type="hidden" value="<%#Eval("JobLevelPerformanceIndicatorID") %>" bindingfield="JobLevelPerformanceIndicatorID" />
