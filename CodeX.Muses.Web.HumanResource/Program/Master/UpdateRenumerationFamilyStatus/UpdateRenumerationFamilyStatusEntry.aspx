<%@ Page Title="" Language="C#" MasterPageFile="~/libs/MasterPage/MPTrx.master" AutoEventWireup="true" 
    CodeBehind="UpdateRenumerationFamilyStatusEntry.aspx.cs" Inherits="CodeX.Muses.Web.Inventory.Program.UpdateRenumerationFamilyStatusEntry" %>
    ...
    <script type="text/javascript">
        function onLoad() {
            ...
            $('#lblTransactionNo.lblLink').click(function () {
                openSearchDialog('transfamilystatusrenumerationhd', onGetItemConsumptionFilterExpression(), function (value) {
            ...
            $('#divTransactionAdd').click(function (evt) {
                if (IsValid(evt, 'fsMPEntry', 'mpEntry')) {
                    editedLineAmount = 0;
                    tacFamilyStatusID.setValue('');
                    tacFamilyStatusID.setText('');
        ...
        $('#<%=grdView.ClientID %> .divDetailDelete').live('click', function () {
            ...
            showToastConfirmation('Are You Sure Want To Delete?', function (result) {
                if (result) {
                    ...
                    $('#<%=hdnEntryID.ClientID %>').val(entity.FamilyStatusID);
        ...
        function onAfterSaveRecordDtSuccess(TransactionID) {
            if ($('#<%=hdnTransactionID.ClientID %>').val() == '0') {
                ...
                Methods.getObject('GetTransFamilyStatusRenumerationHdList', filterExpression, function (result) {
        ...
        function onGetFamilyStatusFilterExpression() {
            var TransactionID = $('#<%=hdnTransactionID.ClientID %>').val();
            var filterExpression = "IsDeleted = 0 AND FamilyStatusID NOT IN (SELECT FamilyStatusID FROM TransFamilyStatusRenumerationDt where TransactionID = " + TransactionID + ")";
            return filterExpression;
        }

        function ontacFamilyStatusIDSearchClick() {
            openSearchDialog('familystatus', onGetFamilyStatusFilterExpression(), function (value) {
                var filterExpression = onGetFamilyStatusFilterExpression() + " AND FamilyStatusID = '" + value + "'";
                Methods.getObject('GetvFamilyStatusList', filterExpression, function (result) {
                    if (result != null) {
                        tacFamilyStatusID.setValue(result.FamilyStatusID);
                        tacFamilyStatusID.setText(result.FamilyStatusName);
                    }
                    else {
                        tacFamilyStatusID.setValue('');
                        tacFamilyStatusID.setText('');
        ...
        function ontacFamilyStatusIDValueChanged() {
        }
        //#endregion
    </script>    
                                    ...
                                    <tr>
                                        <td valign="top">
                                            <table style="width: 100%">
                                                ...
                                                <tr>
                                                <td class="tdLabel"><label class="lblMandatory" id="lblFamilyStatus"><%=GetLabel("Status Pernikahan")%></label></td>
                                                 <td>
                                                    <cdx:CodeXAutoCompleteTextBox runat="server" Width="300px" ID="tacFamilyStatusID" ClientInstanceName="tacFamilyStatusID" MethodName="GetvFamilyStatusList" GetFilterExpressionFunction="onGetFamilyStatusFilterExpression"
                                                        SearchFields="FamilyStatusName,FamilyStatusID" TextField="FamilyStatusName" ValueField="FamilyStatusID" SearchText="${FamilyStatusName} (<b>${MaritalStatus}</b>)" OrderByExpression="FamilyStatusName">
                                                        <ClientSideEvents ButtonSearchClick="function(){ ontacFamilyStatusIDSearchClick(); }"
                                                            ValueChanged="function(){ ontacFamilyStatusIDValueChanged(); }" />
                                                    </cdx:CodeXAutoCompleteTextBox>   
                                                </td>
                                                </tr>
                                            </table>
                            ...
                            <dx:PanelContent ID="PanelContent1" runat="server">
                                <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto;
                                    ...
                                    <asp:GridView ID="grdView" runat="server" CssClass="tblTransactionEntryResult"
                                        ...
                                        <Columns>
                                            <asp:BoundField DataField="FamilyStatusName" HeaderText="Nama" />
                                            <asp:TemplateField HeaderStyle-Width="80px" ItemStyle-HorizontalAlign="Center">
                                                <ItemTemplate>
                                                    <div style='float:right;<%#IsEditable().ToString() == "False" ? "display:none" : "" %>' class="divDetailDelete"></div>
                                                    <input type="hidden" value="<%#Eval("FamilyStatusID") %>" bindingfield="FamilyStatusID" />
                                                    <input type="hidden" value="<%#Eval("FamilyStatusName") %>" bindingfield="FamilyStatusName" />
