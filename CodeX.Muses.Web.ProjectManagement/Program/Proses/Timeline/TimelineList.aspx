<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MPProjectManagementPageTrx.master" AutoEventWireup="true" 
    CodeBehind="TimelineList.aspx.cs" Inherits="CodeX.Muses.Web.ProjectManagement.Program.TimelineList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    ...
    <script type="text/javascript">
        ...
        function refresh() {
            var filterExpression = "HistoryID IN (SELECT HistoryID FROM (SELECT ROW_NUMBER() OVER (ORDER BY HistoryID DESC) AS 'RowNumber',HistoryID FROM ActivityHistory) a WHERE a.RowNumber BETWEEN " + (((currScroll - 1) * rowNumber) + 1) + " AND " + (currScroll * rowNumber) + ")";
            Methods.getListObject('GetvActivityHistoryList', filterExpression, function (result) {
                if (result != null) {
        ...
        function addData() {
            var filterExpression = "HistoryID IN (SELECT HistoryID FROM (SELECT ROW_NUMBER() OVER (ORDER BY HistoryID DESC) AS 'RowNumber',HistoryID FROM ActivityHistory WHERE CreatedDate <= '" + dateTime + "') a WHERE a.RowNumber BETWEEN " + (((currScroll - 1) * rowNumber) + 1) + " AND " + (currScroll * rowNumber) + ")";
            Methods.getListObject('GetvActivityHistoryList', filterExpression, function (result) {
                if (result != null) {
                    if (result.length == 0) isRefresh = false;
                    for (var i = 0; i < result.length; i++) {
