<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="OrganizationDtEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ProjectManagement.Program.OrganizationDtEntryCtl" %>
...
<script type="text/javascript" id="dxss_serviceunitsiteentryctl">
    ...
    $('#<%=grdView.ClientID %> .divDetailEdit').live('click', function () {
        ...
        $('#<%=hdnReportTo.ClientID %>').val(entity.ReportTo);
        tacReportTo.setValue(entity.ReportTo);
        tacReportTo.setText(entity.ReportToPosition);
        if (entity.IsHeader.toString() == "True") $('#<%=chkIsHeader.ClientID %>').attr('checked', true);
        else $('#<%=chkIsHeader.ClientID %>').attr('checked', false);
    ...
    function onTacReportToButtonSearchClick() {
        openSearchDialog('teamdt', onGetTeamDtFilterExpression(), function (value) {
            ...
            Methods.getObject('GetvTeamDtList', filterExpression, function (result) {
                if (result != null) {
                    tacReportTo.setValue(result.TeamDtID);
                    tacReportTo.setText(result.Position);
                    entityToControlReportTo(result);
                }
                else {
                    tacReportTo.setValue('');
                    tacReportTo.setText('');
                    entityToControlReportTo(null);
                }
            });
        });
    ...
    function onTacReportToValueChanged() {
        var id = tacReportTo.getValue();
        if (id != '') {
    ...
    function entityToControlReportTo(result) {
        if (result != null)
            $('#<%=hdnReportTo.ClientID %>').val(result.TeamDtID);
        else
            $('#<%=hdnReportTo.ClientID %>').val('');
    }
    //#endregion
        ...
        <div id="entryDetailContainerPopup" class="entryDetailContainer" style="display: none">
            <fieldset id="fsTrxPopup" style="margin:0"> 
                ...
                <table id="tblEntryPopup">
                    ...
                    <tr>
                        <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Report To")%></label></td>
                        <td>
                            <input type="hidden" id="hdnReportTo" value="" runat="server" />
                            <cdx:CodeXAutoCompleteTextBox runat="server" Width="200px" ID="tacReportTo" ClientInstanceName="tacReportTo" MethodName="GetTeamDtList" GetFilterExpressionFunction="onGetTeamDtFilterExpression"
                                SearchFields="Position" TextField="Position" ValueField="TeamDtID" SearchText="${Position}" OrderByExpression="DisplayOrder">
                                <ClientSideEvents ButtonSearchClick="function(){ onTacReportToButtonSearchClick(); }"
                                    ValueChanged="function(){ onTacReportToValueChanged(); }" />
                            </cdx:CodeXAutoCompleteTextBox>   
                        </td>
                    </tr>
                    <tr>
                        ...
                        <Columns>
                            ...
                            <asp:TemplateField HeaderStyle-Width="80px" ItemStyle-HorizontalAlign="Center">
                                <ItemTemplate>
                                    ...
                                    <input type="hidden" value="<%#Eval("ReportTo") %>" bindingfield="ReportTo" />
                                    <input type="hidden" value="<%#Eval("ReportToPosition") %>" bindingfield="ReportToPosition" />
