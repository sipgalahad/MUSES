<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ProjectOrganizationEntryCtl.ascx.cs" 
    Inherits="CodeX.DTRACK.Web.ProjectManagement.Program.ProjectOrganizationEntryCtl" %>
...
<script type="text/javascript" id="dxss_serviceunitsiteentryctl">
    ...
    $('.divDetailPopupDelete').live('click', function () {
        ...
        $('#<%=hdnTeamID.ClientID %>').val(entity.TeamID);
        cbpProcessPopup.PerformCallback('delete');
    });

    //#region Organization
    window.onGetTeamFilterExpression = function () {
        var filterExpression = "IsDeleted = 0";
        return filterExpression;
    }

    function onTacTeamButtonSearchClick() {
        openSearchDialog('teamhd', onGetTeamFilterExpression(), function (value) {
            var filterExpression = onGetTeamFilterExpression() + " AND TeamCode = '" + value + "'";
            Methods.getObject('GetvTeamHdList', filterExpression, function (result) {
                if (result != null) {
                    tacTeam.setValue(result.TeamID);
                    tacTeam.setText(result.TeamName);
                    $('#<%=hdnTeamID.ClientID %>').val(result.TeamID);
                }
                else {
                    tacTeam.setValue('');
                    tacTeam.setText('');
                    $('#<%=hdnTeamID.ClientID %>').val('');
                }
            });
        });
    }

    function onTacTeamValueChanged() {
        ...
        <div id="entryDetailContainerPopup" class="entryDetailContainer" style="display: none">
            <fieldset id="fsTrxPopup" style="margin:0"> 
                ...
                <table id="tblEntryPopup">
                    ...
                    <tr>
                        <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Team")%></label></td>
                        <td>
                            <input type="hidden" id="hdnTeamID" value="" runat="server" />
                            <cdx:CodeXAutoCompleteTextBox runat="server" Width="200px" ID="tacTeam" ClientInstanceName="tacTeam" MethodName="GetTeamList" GetFilterExpressionFunction="onGetTeamFilterExpression"
                                SearchFields="TeamName,TeamCode" TextField="TeamName" ValueField="TeamID" SearchText="${TeamName} (<b>${TeamCode}</b>)" OrderByExpression="TeamName">
                                <ClientSideEvents ButtonSearchClick="function(){ onTacTeamButtonSearchClick(); }"
                                    ValueChanged="function(){ onTacTeamValueChanged(); }" />
            ...
            <dx:PanelContent ID="PanelContent1" runat="server">
                <asp:Panel runat="server" ID="pnlPatientVisitTransHdGrdView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                    <asp:GridView ID="grdPopupView" runat="server" CssClass="tblTransactionEntryResult" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                        <Columns>
                            <asp:BoundField DataField="TeamCode" HeaderText="Code"/>
                            <asp:BoundField DataField="TeamName" HeaderText="Nama"/>
                            <asp:TemplateField HeaderStyle-Width="80px" ItemStyle-HorizontalAlign="Center">
                                <ItemTemplate>
                                    <div style='float:right;' class="divDetailPopupDelete">X</div>
                                    <input type="hidden" value="<%#Eval("TeamID") %>" bindingfield="TeamID" />
                                    <input type="hidden" value="<%#Eval("TeamCode") %>" bindingfield="TeamCode" />
                                    <input type="hidden" value="<%#Eval("TeamName") %>" bindingfield="TeamName" />
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                        <EmptyDataTemplate>
