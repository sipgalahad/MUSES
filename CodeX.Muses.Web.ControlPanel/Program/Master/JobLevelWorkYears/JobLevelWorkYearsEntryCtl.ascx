<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="JobLevelWorkYearsEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.JobLevelWorkYearsEntryCtl" %>
...
<script type="text/javascript" id="dxss_serviceunitsiteentryctl">
    $(function () {
        $('#divTransactionAddPopup').click(function () {
            ...
            $('#<%=hdnJobLevelID.ClientID %>').val('');
            $('#<%=hdnDeleteJobLevelID.ClientID %>').val('');
    ...
    $('#<%=grdView.ClientID %> .divDetailDelete').live('click', function () {
        ...
        showToastConfirmation("Are You Sure Want To Delete This Data?", function (result) {
            if (result) {
                ...
                $('#<%=hdnEntryID.ClientID %>').val(entity.JobLevelWorkYearsID);
                $('#<%=hdnDeleteJobLevelID.ClientID %>').val(entity.JobLevelID);
    ...
    $('#<%=grdView.ClientID %> .divDetailEdit').live('click', function () {
        ...
        $('#<%=hdnEntryID.ClientID %>').val(entity.JobLevelWorkYearsID);
        tacJobLevel.setValue(entity.JobLevelID);
        tacJobLevel.setText(entity.JobLevelName);
        $('#<%=hdnJobLevelID.ClientID %>').val(entity.JobLevelID);
        $('#entryDetailContainerPopup').show();
    });
    ...
    function ontacJobLevelSearchClick() {
        openSearchDialog('employee', onGetEmployeeFilterExpression(), function (value) {
            var filterExpression = " IsDeleted = 0 ";
            Methods.getObject('GeJobLevelList', filterExpression, function (result) {
                if (result != null) {
                    $('#<%=hdnJobLevelID.ClientID %>').val(result.JobLevelID);
                    tacJobLevel.setValue(result.JobLevelID);
                    tacJobLevel.setText(result.JobLevelName);
                }
                else {
                    $('#<%=hdnJobLevelID.ClientID %>').val('');
                    tacJobLevel.setValue('');
                    tacJobLevel.setText('');
                }
            });
        });
    ...
    function ontacJobLevelValueChanged() {
    }
    //#endregion
...
<div style="height:440px; overflow-y:auto">
    ...
    <input type="hidden" id="hdnDeleteJobLevelID" runat="server" />
    <table class="tblEntryContent" style="width:70%">
        ...
        <tr>
            <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Keterangan")%></label></td>
            <td colspan="2"><asp:TextBox ID="txtKeterangan" ReadOnly="true" Width="100%" runat="server" /></td>
        </tr> 
    </table>
            ...
            <fieldset id="fsTrxPopup" style="margin:0"> 
                ...
                <table>
                    ...
                    <tr>
                         ...
                         <td>
                            <input type="hidden" id="hdnJobLevelID" runat="server" />
                            <cdx:CodeXAutoCompleteTextBox runat="server" Width="200px" ID="tacJobLevel" ClientInstanceName="tacJobLevel" MethodName="GetvJobLevelList" GetFilterExpressionFunction="onGetEmployeeFilterExpression"
                                SearchFields="JobLevelName,JobLevelCode" TextField="JobLevelName" ValueField="JobLevelID" SearchText="${JobLevelName} (<b>${JobLeveleCode}</b>)" OrderByExpression="JobLName">
                                <ClientSideEvents ButtonSearchClick="function(){ onTacJobLevelSearchClick(); }"
                                    ValueChanged="function(){ onTacJobLevelValueChanged(); }" />
            ...
            <dx:PanelContent ID="PanelContent1" runat="server">
                <asp:Panel runat="server" ID="pnlPatientVisitTransHdGrdView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                    <asp:GridView ID="grdView" runat="server" CssClass="tblTransactionEntryResult" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                        <Columns>
                            <asp:BoundField DataField="PerformanceIndicatorDtID" HeaderStyle-CssClass="keyField" ItemStyle-CssClass="keyField" />
                            <asp:BoundField DataField="PerformanceIndicatorDtName" HeaderText="Nama" HeaderStyle-CssClass="thLeft" ItemStyle-HorizontalAlign="Left" HeaderStyle-Width="100px" />
