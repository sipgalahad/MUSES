<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="LocationPermissionDtEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.LocationPermissionDtEntryCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{TransactionCode}/g, $selectedTr.find('.tdTransactionCode').html());
            $newTr = $newTr.replace(/\$\{TransactionName}/g, $selectedTr.find('.tdTransactionName').html());
            $newTr = $newTr.replace(/\$\{TransactionCode}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
...
<div style="padding:10px;">
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${TransactionCode}' />
            </td>
            <td class="tdTransactionCode">${TransactionCode}</td>
            <td>${TransactionName}</td>
        </tr>
    </script>
    <input type="hidden" id="hdnSelectedMember" runat="server" value="" />
    <input type="hidden" id="hdnRestrictionID" runat="server" value="" />
    ...
    <table class="tblEntryContent" style="width:70%">
        ...
        <tr>
            ...
            <td><asp:TextBox ID="txtRestrictionName" ReadOnly="true" Width="200px" runat="server" /></td>
        </tr> 
    </table>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="TransactionCode" HeaderText="Kode" ItemStyle-CssClass="tdTransactionCode" HeaderStyle-Width="50px" />
                                        <asp:BoundField DataField="TransactionName" HeaderText="Nama" ItemStyle-CssClass="tdTransactionName" />
                                    </Columns>
                                    <EmptyDataTemplate>
                        ...
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    <td align="center">
                                        <input type="checkbox" class="chkIsSelected2" />
                                        <input type="hidden" class="keyField" value='<%#Eval("TransactionCode") %>' />
                                    </td>
                                    <td class="tdTransactionCode"><%#Eval("TransactionCode") %></td>
