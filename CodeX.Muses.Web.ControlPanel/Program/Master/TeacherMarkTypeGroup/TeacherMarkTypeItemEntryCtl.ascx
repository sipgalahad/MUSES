<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TeacherMarkTypeItemEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.TeacherMarkTypeItemEntryCtl" %>
...
<script type="text/javascript" id="dxss_serviceunitsiteentryctl">
    $(function () {
        $('#divTransactionAddPopup').click(function () {
            ...
            $('#<%=txtTeacherMarkTypeItemName.ClientID %>').val('');
            $('#<%=txtFinalMark.ClientID %>').val('');
            $('#<%=txtDisplayOrder.ClientID %>').val('');
            $('#<%=txtKeyProgressIndicatorText.ClientID %>').val('');
            cboDimension.SetValue('');
    ...
    $('#<%=grdView.ClientID %> .divDetailDelete').live('click', function () {
        ...
        showToastConfirmation("Are You Sure Want To Delete This Data?", function (result) {
            if (result) {
                ...
                $('#<%=hdnEntryID.ClientID %>').val(entity.TeacherMarkTypeItemID);
    ...
    $('#<%=grdView.ClientID %> .divDetailEdit').live('click', function () {
        ...
        var entity = rowToObject($row);
        $('#<%=hdnEntryID.ClientID %>').val(entity.TeacherMarkTypeItemID);
        $('#<%=txtTeacherMarkTypeItemName.ClientID %>').val(entity.TeacherMarkTypeItemName);
        $('#<%=txtFinalMark.ClientID %>').val(entity.FinalMarkPercentage);
        $('#<%=txtDisplayOrder.ClientID %>').val(entity.DisplayOrder);
        $('#<%=txtKeyProgressIndicatorText.ClientID %>').val(entity.KeyProgressIndicatorText);
        cboDimension.SetValue(entity.TeacherMarkTypeDimensionID);
        $('#entryDetailContainerPopup').show();
    });
            ...
            <fieldset id="fsTrxPopup" style="margin:0"> 
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td><dxe:ASPxComboBox runat="server" ID="cboDimension" ClientInstanceName="cboDimension" Width="200px" /></td>
                    </tr>
                    <tr>
                        <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Nama Sub Dimensi")%></label></td>
                        <td><asp:TextBox runat="server" ID="txtTeacherMarkTypeItemName" Width="200px" /></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="tdLabel" style="vertical-align: top; padding-top: 5px;"><label class="lblNormal"><%=GetLabel("KPI")%></label></td>
                        <td><asp:TextBox ID="txtKeyProgressIndicatorText" Width="400px" runat="server" TextMode="MultiLine" Rows="2" /></td>
                    </tr>
                    <tr>
                        <td> 
            ...
            <dx:PanelContent ID="PanelContent1" runat="server">
                <asp:Panel runat="server" ID="pnlPatientVisitTransHdGrdView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                    <asp:GridView ID="grdView" runat="server" CssClass="tblTransactionEntryResult" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                        <Columns>
                            <asp:BoundField DataField="TeacherMarkTypeDimensionName" HeaderText="Dimensi" HeaderStyle-Width="120px"/>
                            <asp:BoundField DataField="TeacherMarkTypeItemName" HeaderText="Nama" HeaderStyle-Width="200px"/>
                            ...
                            <asp:BoundField DataField="KeyProgressIndicatorText" HeaderText="Indikator Kunci Kinerja (KPI)"  />
                            <asp:TemplateField HeaderStyle-Width="80px" ItemStyle-HorizontalAlign="Center">
                                <ItemTemplate>
                                    <div style='float:right;' class="divDetailDelete"></div>
                                    <div style='float:right;margin-right:10px;' class="divDetailEdit"><%=GetLabel("Edit")%></div>
                                    <input type="hidden" value="<%#Eval("TeacherMarkTypeItemID") %>" bindingfield="TeacherMarkTypeItemID" />
                                    <input type="hidden" value="<%#Eval("TeacherMarkTypeItemName") %>" bindingfield="TeacherMarkTypeItemName" />
                                    ...
                                    <input type="hidden" value="<%#Eval("TeacherMarkTypeDimensionID") %>" bindingfield="TeacherMarkTypeDimensionID" />
                                    <input type="hidden" value="<%#Eval("KeyProgressIndicatorText") %>" bindingfield="KeyProgressIndicatorText" />
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>
                        <EmptyDataTemplate>
