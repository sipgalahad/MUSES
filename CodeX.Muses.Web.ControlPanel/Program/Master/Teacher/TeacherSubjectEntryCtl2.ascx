<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TeacherSubjectEntryCtl2.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.TeacherSubjectEntryCtl2" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{SubjectCode}/g, $selectedTr.find('.tdSubjectCode').html());
            $newTr = $newTr.replace(/\$\{SubjectName}/g, $selectedTr.find('.tdSubjectName').html());
            $newTr = $newTr.replace(/\$\{SubjectID}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
...
<div style="padding:10px;">
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${SubjectID}' />
            </td>
            <td class="tdSubjectCode">${SubjectCode}</td>
            <td>${SubjectName}</td>
        </tr>
    </script>
    <input type="hidden" id="hdnSelectedMember" runat="server" value="" />
    <input type="hidden" id="hdnTeacherID" runat="server" value="" />
    ...
    <table class="tblEntryContent" style="width:70%">
        ...
        <tr>
            ...
            <td><asp:TextBox ID="txtTeacherName" ReadOnly="true" Width="200px" runat="server" /></td>
        </tr> 
    </table>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="SubjectCode" HeaderText="Kode" ItemStyle-CssClass="tdSubjectCode" HeaderStyle-Width="50px" />
                                        <asp:BoundField DataField="SubjectName" HeaderText="Nama" ItemStyle-CssClass="tdSubjectName" />
                                    </Columns>
                                    <EmptyDataTemplate>
                        ...
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    <td align="center">
                                        <input type="checkbox" class="chkIsSelected2" />
                                        <input type="hidden" class="keyField" value='<%#Eval("SubjectID") %>' />
                                    </td>
                                    <td class="tdSubjectCode"><%#Eval("SubjectCode") %></td>
