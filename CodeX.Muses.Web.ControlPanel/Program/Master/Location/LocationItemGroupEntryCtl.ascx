<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="LocationItemGroupEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.LocationItemGroupEntryCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{ItemGroupCode}/g, $selectedTr.find('.tdItemGroupCode').html());
            $newTr = $newTr.replace(/\$\{ItemGroupName1}/g, $selectedTr.find('.tdItemGroupName1').html());
            $newTr = $newTr.replace(/\$\{ItemGroupID}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
    ...
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${ItemGroupID}' />
            </td>
            <td class="tdItemGroupCode">${ItemGroupCode}</td>
            <td>${ItemGroupName1}</td>
        </tr>
    </script>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="ItemGroupCode" HeaderText="Kode" ItemStyle-CssClass="tdItemGroupCode" HeaderStyle-Width="50px" />
                                        <asp:TemplateField ItemStyle-CssClass="tdItemGroupName1">
                                            <HeaderTemplate>
                                                <div style="padding-left:3px">
                                                    <%=GetLabel("Item Group Name")%>
                                                </div>
                                            </HeaderTemplate>
                                            <ItemTemplate>
                                                <div style='margin-left:<%# Eval("Level") %>0px;'><%# Eval("ItemGroupName1")%></div>
                                            </ItemTemplate>
                                        </asp:TemplateField>
                                    </Columns>
                                    <EmptyDataTemplate>
                        ...
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    <td align="center">
                                        <input type="checkbox" class="chkIsSelected2" />
                                        <input type="hidden" class="keyField" value='<%#Eval("ItemGroupID") %>' />
                                    </td>
                                    <td class="tdItemGroupCode"><%#Eval("ItemGroupCode") %></td>
