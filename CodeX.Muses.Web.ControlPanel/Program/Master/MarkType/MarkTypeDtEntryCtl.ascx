<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="MarkTypeDtEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.MarkTypeDtEntryCtl" %>
...
<script type="text/javascript" id="dxss_serviceunitsiteentryctl">
    $(function () {
        $('#divTransactionAddPopup').click(function () {
            ...
            $('#<%=txtMarkTypeDtName.ClientID %>').val('');
            $('#<%=txtDisplayOrder.ClientID %>').val('');
            $('#<%=txtRemarks.ClientID %>').val('');

            $('#entryDetailContainerPopup').show();
            $('#<%=txtMarkTypeDtName.ClientID %>').focus();
        });
    ...
    $('#<%=grdView.ClientID %> .divDetailDelete').live('click', function () {
        ...
        showToastConfirmation("Are You Sure Want To Delete This Data?", function (result) {
            if (result) {
                ...
                $('#<%=hdnEntryID.ClientID %>').val(entity.MarkTypeDtID);
    ...
    $('#<%=grdView.ClientID %> .divDetailEdit').live('click', function () {
        ...
        var entity = rowToObject($row);
        $('#<%=hdnEntryID.ClientID %>').val(entity.MarkTypeDtID);
        $('#<%=txtMarkTypeDtName.ClientID %>').val(entity.MarkTypeDtName);
        ...
        <div id="entryDetailContainerPopup" class="entryDetailContainer" style="display: none">
            <fieldset id="fsTrxPopup" style="margin:0"> 
                ...
                <table>
                    ...
                    <tr>
                        <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Teks")%></label></td>
                        <td><asp:TextBox ID="txtMarkTypeDtName" runat="server" Width="150px" /></td>
                    </tr>
                    <tr>
            ...
            <dx:PanelContent ID="PanelContent1" runat="server">
                <asp:Panel runat="server" ID="pnlPatientVisitTransHdGrdView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                    <asp:GridView ID="grdView" runat="server" CssClass="tblTransactionEntryResult" AutoGenerateColumns="false" ShowHeaderWhenEmpty="true" EmptyDataRowStyle-CssClass="trEmpty">
                        <Columns>
                            <asp:BoundField DataField="MarkTypeDtName" HeaderText="Teks" HeaderStyle-Width="200px" />
                            <asp:BoundField DataField="Remarks" HeaderText="Keterangan" />
                            <asp:TemplateField HeaderStyle-Width="80px" ItemStyle-HorizontalAlign="Center">
                                <ItemTemplate>
                                    <div style='float:right;' class="divDetailDelete"></div>
                                    <div style='float:right;margin-right:10px;' class="divDetailEdit"><%=GetLabel("Edit")%></div>
                                    <input type="hidden" value="<%#Eval("MarkTypeDtID") %>" bindingfield="MarkTypeDtID" />
                                    <input type="hidden" value="<%#Eval("MarkTypeDtName") %>" bindingfield="MarkTypeDtName" />
