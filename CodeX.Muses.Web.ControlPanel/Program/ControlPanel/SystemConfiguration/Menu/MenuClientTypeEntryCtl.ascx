<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="MenuClientTypeEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.MenuClientTypeEntryCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{GCClientType}/g, $selectedTr.find('.tdGCClientType').html());
            $newTr = $newTr.replace(/\$\{ClientType}/g, $selectedTr.find('.tdClientType').html());
            $newTr = $newTr.replace(/\$\{GCClientType}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
...
<div style="padding:10px;">
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${GCClientType}' />
            </td>
            <td class="tdGCClientType">${GCClientType}</td>
            <td>${ClientType}</td>
        </tr>
    </script>
    ...
    <table class="tblEntryContent" style="width:70%">
        ...
        <tr>
            ...
            <td><asp:TextBox ID="txtMenuName" ReadOnly="true" Width="200px" runat="server" /></td>
        </tr> 
    </table>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="cfStandardCodeID" HeaderText="Kode" ItemStyle-CssClass="tdGCClientType" HeaderStyle-Width="50px" />
                                        <asp:BoundField DataField="StandardCodeName" HeaderText="Nama" ItemStyle-CssClass="tdClientType" />
                                    </Columns>
                                    <EmptyDataTemplate>
                        ...
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    <td align="center">
                                        <input type="checkbox" class="chkIsSelected2" />
                                        <input type="hidden" class="keyField" value='<%#Eval("cfGCClientType") %>' />
                                    </td>
                                    <td class="tdGCClientType"><%#Eval("cfGCClientType")%></td>
