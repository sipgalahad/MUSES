<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UserRolesTreasuryBookEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.UserRolesTreasuryBookEntryCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{BookCode}/g, $selectedTr.find('.tdBookCode').html());
            $newTr = $newTr.replace(/\$\{BookName}/g, $selectedTr.find('.tdBookName').html());
...
<div style="padding:10px;">
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            ...
            <td class="tdBookCode">${BookCode}</td>
            <td>${BookName}</td>
        </tr>
    </script>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="BookCode" HeaderText="Kode" ItemStyle-CssClass="tdBookCode" HeaderStyle-Width="50px" />
                                        <asp:BoundField DataField="BookName" HeaderText="Nama" ItemStyle-CssClass="tdBookName" />
                                    </Columns>
                                    <EmptyDataTemplate>
                        ...
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    ...
                                    <td class="tdBookCode"><%#Eval("BookCode") %></td>
                                    <td><%#Eval("BookName") %></td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
