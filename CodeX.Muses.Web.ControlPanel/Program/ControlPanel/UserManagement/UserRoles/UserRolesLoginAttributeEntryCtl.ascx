<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UserRolesLoginAttributeEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.UserRolesLoginAttributeEntryCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{LoginAttributeCode}/g, $selectedTr.find('.tdLoginAttributeCode').html());
            $newTr = $newTr.replace(/\$\{LoginAttributeName}/g, $selectedTr.find('.tdLoginAttributeName').html());
            $newTr = $newTr.replace(/\$\{LoginAttributeID}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
    ...
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${LoginAttributeID}' />
            </td>
            <td class="tdLoginAttributeCode">${LoginAttributeCode}</td>
            <td>${LoginAttributeName}</td>
        </tr>
    </script>
                        ...
                        <dx:PanelContent ID="PanelContent1" runat="server">
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        <asp:BoundField DataField="LoginAttributeID" HeaderStyle-CssClass="keyField" ItemStyle-CssClass="keyField"/>
                                        <asp:TemplateField HeaderStyle-Width="40px" ItemStyle-HorizontalAlign="Center" HeaderStyle-HorizontalAlign="Center">
                                            <ItemTemplate>
                                                <asp:CheckBox ID="chkIsSelected" runat="server" CssClass="chkIsSelected" />
                                            </ItemTemplate>
                                        </asp:TemplateField>
                                        <asp:BoundField DataField="LoginAttributeCode" HeaderText="Kode" ItemStyle-CssClass="tdLoginAttributeCode" HeaderStyle-Width="50px" />
                                        <asp:BoundField DataField="LoginAttributeName" HeaderText="Nama" ItemStyle-CssClass="tdLoginAttributeName" />
                                    </Columns>
                                    <EmptyDataTemplate>
                        ...
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    <td align="center">
                                        <input type="checkbox" class="chkIsSelected2" />
                                        <input type="hidden" class="keyField" value='<%#Eval("LoginAttributeID") %>' />
                                    </td>
                                    <td class="tdLoginAttributeCode"><%#Eval("LoginAttributeCode") %></td>
                                    <td><%#Eval("LoginAttributeName") %></td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
                        <tr id="trFooter"></tr>
