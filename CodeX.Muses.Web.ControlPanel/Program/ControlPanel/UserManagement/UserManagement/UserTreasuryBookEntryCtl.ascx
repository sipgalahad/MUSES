<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UserTreasuryBookEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanel.Program.UserTreasuryBookEntryCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{TreasuryBookCode}/g, $selectedTr.find('.tdTreasuryBookCode').html());
            $newTr = $newTr.replace(/\$\{TreasuryBookName}/g, $selectedTr.find('.tdTreasuryBookName').html());
            $newTr = $newTr.replace(/\$\{BookID}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
    ...
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${BookID}' />
            </td>
            <td class="tdTreasuryBookCode">${TreasuryBookCode}</td>
            <td>${TreasuryBookName}</td>
        </tr>
    </script>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="TreasuryBookCode" HeaderText="Kode" ItemStyle-CssClass="tdTreasuryBookCode" HeaderStyle-Width="50px" />
                                        <asp:BoundField DataField="TreasuryBookName" HeaderText="Nama" ItemStyle-CssClass="tdTreasuryBookName" />
                                    </Columns>
                                    <EmptyDataTemplate>
                        ...
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    <td align="center">
                                        <input type="checkbox" class="chkIsSelected2" />
                                        <input type="hidden" class="keyField" value='<%#Eval("BookID") %>' />
                                    </td>
                                    <td class="tdTreasuryBookCode"><%#Eval("TreasuryBookCode") %></td>
                                    <td><%#Eval("TreasuryBookName") %></td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
                        <tr id="trFooter"></tr>
