<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="APInvoiceSupplierProcessPurchaseReturnCtl.ascx.cs" 
    Inherits="CodeX.Ottimo.Web.Finance.Program.APInvoiceSupplierProcessPurchaseReturnCtl" %>
...
<script type="text/javascript" id="dxss_apinvoicesupplierprocessctl">
    function getCheckedPurchaseReceive() {
        var lstSelectedPurchaseReturn = $('#<%=hdnSelectedPurchaseReturn.ClientID %>').val().split(',');
        $('.chkPurchaseReturn input').each(function () {
            if ($(this).is(':checked')) {
        ...
        $('#<%=hdnSelectedPurchaseReturn.ClientID %>').val(lstSelectedPurchaseReturn.join(','));
    }

    $('#chkSelectAllPR').die('change');
    $('#chkSelectAllPR').live('change', function () {
        var isChecked = $(this).is(":checked");
        $('.chkPurchaseReturn').each(function () {
            $chk = $(this).find('input');
            if (!$chk.is(":disabled")) {
    ...
    function onBeforeSaveRecordPopup(errMessage) {
        ...
        if ($('#<%=hdnSelectedPurchaseReturn.ClientID %>').val() == '') {
            errMessage.text = 'Please Select Purchase Return First';
            return false;
        }
        return true;
    }
...
<div style="height:440px; overflow-y:auto;overflow-x: hidden">
    <input type="hidden" id="hdnSelectedPurchaseReturn" runat="server" value="" />
                            ...
                            <asp:Panel runat="server" ID="pnlEntryPopupGrdView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                <asp:ListView runat="server" ID="lvwView" OnItemDataBound="lvwView_ItemDataBound">
                                    <EmptyDataTemplate>
                                        <table id="tblView" runat="server" class="tblTransactionEntryResult" cellspacing="0" rules="all" >
                                            <tr>
                                                ...
                                                <th style="width:180px" align="center"><%=GetLabel("No. Pengembalian")%></th>
                                                <th style="width:150px" align="center"><%=GetLabel("No. Faktur")%></th>
                                                <th style="width:160px" class="thRight"><%=GetLabel("Jumlah Pengembalian")%></th>
                                            </tr>
                                            <tr class="trEmpty">
                                                <td colspan="6">
                                    ...
                                    <ItemTemplate>
                                        <tr>
                                            <td align="center">
                                                <asp:CheckBox ID="chkPurchaseReturn" runat="server" CssClass="chkPurchaseReturn" />
                                                <input type="hidden" class="keyField" id="keyField" runat="server" value='<%# Eval("PurchaseReturnID")%>' />
                                            </td>
                                            <td><%# Eval("PurchaseReceiveNo")%></td>
                                            <td><%# Eval("PurchaseReturnNo")%></td>
