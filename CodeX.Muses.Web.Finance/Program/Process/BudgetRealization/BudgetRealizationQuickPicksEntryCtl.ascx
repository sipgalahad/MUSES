<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="BudgetRealizationQuickPicksEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.Finance.Program.BudgetRealizationQuickPicksEntryCtl" %>
...
<script type="text/javascript" id="dxss_testorderquickpicksctl">
    ...
    function getCheckedMember() {
        ...
        $('#tblSelectedItem .trSelectedItem').each(function () {
            ...
            var realization = $(this).find('.txtRealizationAmount').attr('hiddenVal');
            lstSelectedMember.push(key);
            lstSelectedMemberQty.push(realization);
        });

        $('#<%=hdnSelectedMember.ClientID %>').val(lstSelectedMember.join(','));
        $('#<%=hdnSelectedRequest.ClientID %>').val(lstSelectedMemberQty.join(','));
    }
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{BudgetName}/g, $selectedTr.find('.tdBudgetName').html());
            $newTr = $newTr.replace(/\$\{RequestAmount}/g, $selectedTr.find('.tdRequestAmount').html());
            $newTr = $newTr.replace(/\$\{BudgetRequestDtID}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
    ...
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${BudgetRequestDtID}' />
            </td>
            <td>${BudgetName}</td>
            <td align="right">${RequestAmount}</td>
            <td><input type="text" value="0" class="txtCurrency txtRealizationAmount" style="width:100px;" /></td>
        </tr>
    </script>
    <input type="hidden" id="hdnSelectedMember" runat="server" value="" />
    <input type="hidden" id="hdnSelectedRequest" runat="server" value="" />
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="BudgetName" HeaderText="Nama" ItemStyle-CssClass="tdBudgetName" />
                                        <asp:BoundField DataField="CustomRequestAmount" HeaderText="Diminta" DataFormatString="{0:N}" HeaderStyle-CssClass="thRight" ItemStyle-HorizontalAlign="Right" HeaderStyle-Width="150px" ItemStyle-CssClass="tdRequestAmount" />
                                    </Columns>
                                    <EmptyDataTemplate>
            ...
            <td style="padding:5px;vertical-align:top">
                ...
                <table id="tblSelectedItem" class="grdView notAllowSelect" cellspacing="0" rules="all" >
                    ...
                    <tr id="trHeader2">
                        ...
                        <th class="thRight"><%=GetLabel("Diminta")%></th>
                        <th class="thRight"><%=GetLabel("Direalisasikan")%></th>
                    </tr>
                </table> 
            </td>
        </tr>
