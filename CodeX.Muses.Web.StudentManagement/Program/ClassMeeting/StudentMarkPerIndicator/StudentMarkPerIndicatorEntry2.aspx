<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MPClassSubjectPageTrxVisit.master" AutoEventWireup="true" 
    CodeBehind="StudentMarkPerIndicatorEntry2.aspx.cs" Inherits="CodeX.Muses.Web.StudentManagement.Program.StudentMarkPerIndicatorEntry2" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="plhEntry" runat="server">
    <script type="text/javascript">
        $(function () {
            ...
            $('#<%=btnSave.ClientID %>').click(function () {6
                var result = '';
                var lstIndicatorName = '';
                var lstIndicator = [];
                $('.hdnIndicatorName').each(function () {
                    var indicatorName = $(this).val();
                    lstIndicator.push(indicatorName);
                    if (lstIndicatorName != '')
                        lstIndicatorName += '|';
                    lstIndicatorName += indicatorName;
                });

                $('.trStudent').each(function () {
                    var tempResult = '';
                    $(this).find('.hdnSubjectIndicatorName').each(function () {
                        var indicatorName = $(this).val();
                        var curriculumMarkTypeID = $(this).parent().find('.hdnCurriculumMarkTypeID').val();
                        var competencyMarkID = '';
                        ...
                        if (cboCompetencyMarkType.GetValue() != null && cboCompetencyMarkType.GetValue() != '0')
                            competencyMarkID = cboCompetencyMarkType.GetValue();
                        if (tempResult != '')
                            tempResult += '*';

                        var indicatorIdx = -1;
                        for (var i = 0; i < lstIndicator.length; ++i) {
                            if (lstIndicator[i] == indicatorName) {
                                indicatorIdx = i;
                                break;
                            }
                        }
                        tempResult += curriculumMarkTypeID + '^' + indicatorIdx + '^' + competencyMarkID;
                    });
                ...
                $('#<%=hdnListIndicatorName.ClientID %>').val(lstIndicatorName);
                    ...
                    <ItemTemplate>
                        <asp:Repeater ID="rptHeader3Dt" runat="server" OnItemDataBound="rptHeader3Dt_ItemDataBound">
                            <ItemTemplate>
                                ...
                                <th class="thCenter" id="thAvgMark" runat="server" style="width: 80px"><%=GetLabel("Rata-Rata")%></th>
                                <th class="thCenter" id="thPredicateMark" runat="server" style="width: 80px"><%=GetLabel("Predikat")%></th>
                            </ItemTemplate>
                        </asp:Repeater>
                    </ItemTemplate>
                </asp:Repeater>
                            ...
                            <ItemTemplate>
                                <asp:Repeater ID="rptStudentMark" runat="server" OnItemDataBound="rptStudentMark_ItemDataBound">
                                    <ItemTemplate>
                                        ...
                                        <td align="center">
                                            <input type="hidden" id="hdnSubjectIndicatorName" runat="server" class="hdnSubjectIndicatorName" />
                                            <input type="hidden" id="hdnCurriculumMarkTypeID" runat="server" class="hdnCurriculumMarkTypeID" />
                                            <dxe:ASPxComboBox ID="cboCompetencyMarkType" runat="server" Width="90%" />
                                        </td>
                                    </ItemTemplate>
                                </asp:Repeater>
                            </ItemTemplate>
