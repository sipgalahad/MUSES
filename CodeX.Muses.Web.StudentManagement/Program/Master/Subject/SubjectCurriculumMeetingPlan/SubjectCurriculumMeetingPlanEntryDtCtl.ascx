<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SubjectCurriculumMeetingPlanEntryDtCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.StudentManagement.Program.SubjectCurriculumMeetingPlanEntryDtCtl" %>
    ...
    function onCbpEntryPopupViewEndCallback(s) {
        ...
        if (param[0] == 'saveclose') {
            ...
            if (param[1] == 'fail')
                ...
                onAfterSaveRecordSubjectCurriculumMeetingPlan();
                $('#<%=txtSubjectCurriculumMeetingPlanCode.ClientID %>').val('');
                $('#<%=txtSubjectCurriculumMeetingPlanName.ClientID %>').val('');
                $('#<%=txtRemarks.ClientID %>').val('');
                $('#<%=hdnIsAdd.ClientID %>').val('1');
                cboReferenceID.SetValue('');
                $('#containerPopupEntryData').show();
                if ($('#<%=hdnIsUsingCode.ClientID %>').val() == '1')
                    $('#<%=txtSubjectCurriculumMeetingPlanCode.ClientID %>').focus();
                else
                    $('#<%=txtSubjectCurriculumMeetingPlanName.ClientID %>').focus();
            }
        }
        hideLoadingPanel();
    }
...
<div>
    ...
    <input type="hidden" id="hdnIsPerSchoolTimeUnit" value="" runat="server" />
    <input type="hidden" id="hdnCurriculumSchoolTimeUnitID" value="" runat="server" />
                ...
                <div id="containerPopupEntryData">
                    <fieldset id="fsEntryPopup" style="margin: 0">
                        <table style="width: 100%" cellpadding="0" cellspacing="1">
                            ...
                            <tr id="trSchoolTimeUnit" runat="server">
                                <td class="tdLabel"><label class="lblNormal"><%=GetLabel("Semester")%></label></td>
                                <td><asp:TextBox ID="txtSchoolTimeUnitName" runat="server" Width="200px" ReadOnly="true" /></td>
                            </tr>
                            <tr id="trCode" runat="server">
                                <td class="tdLabel"><label class="lblMandatory"><%=GetLabel("Kode")%></label></td>
                                <td><asp:TextBox ID="txtSubjectCurriculumMeetingPlanCode" runat="server" Width="100px" /></td>
                            </tr>
                            <tr id="trName" runat="server">
                                <td style="padding-top:5px; vertical-align: top" class="tdLabel"><label class="lblMandatory"><%=GetLabel("Teks")%></label></td>
                                <td><asp:TextBox ID="txtSubjectCurriculumMeetingPlanName" runat="server" Width="450px" TextMode="MultiLine" Rows="5" /></td>
                            </tr>
