<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PurchaseBudgetQuickPicksCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.Inventory.Program.PurchaseBudgetQuickPicksCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr.find('.txtTotalAmount').val('0').trigger('changeValue');
...
<div style="padding:10px;">
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            ...
            <td>${ItemName1}</td>
            <td><input type="text" validationgroup="mpDrugsQuickPicks" class="txtQty number min max" min="1" value="1" style="width:100%" /></td>
            <td><input type="text" validationgroup="mpDrugsQuickPicks" class="txtTotalAmount txtCurrency" value="0" style="width:100%" /></td>
        </tr>
    </script>
    ...
    <input type="hidden" id="hdnSelectedMemberTotalAmount" runat="server" value="" />
    <input type="hidden" id="hdnSiteServiceUnitID" runat="server" value="" />
                ...
                <fieldset id="fsDrugsQuickPicks">
                    <table id="tblSelectedItem" class="grdView notAllowSelect" cellspacing="0" rules="all" >
                        <tr id="trHeader2">
                            ...
                            <th align="center"style="width:100px"><%=GetLabel("Nilai")%></th> 
                        </tr>
                        <tr id="trFooter"></tr>
                    </table>
                </fieldset>
