<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UserRolesLocationEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanelHQ.Program.UserRolesLocationEntryCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{LocationCode}/g, $selectedTr.find('.tdLocationCode').html());
            $newTr = $newTr.replace(/\$\{LocationName}/g, $selectedTr.find('.tdLocationName').html());
            $newTr = $newTr.replace(/\$\{LocationID}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
    ...
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${LocationID}' />
            </td>
            <td class="tdLocationCode">${LocationCode}</td>
            <td>${LocationName}</td>
        </tr>
    </script>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="LocationCode" HeaderText="Kode" ItemStyle-CssClass="tdLocationCode" HeaderStyle-Width="50px" />
                                        <asp:BoundField DataField="LocationName" HeaderText="Nama" ItemStyle-CssClass="tdLocationName" />
                                    </Columns>
                                    <EmptyDataTemplate>
                        ...
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    <td align="center">
                                        <input type="checkbox" class="chkIsSelected2" />
                                        <input type="hidden" class="keyField" value='<%#Eval("LocationID") %>' />
                                    </td>
                                    <td class="tdLocationCode"><%#Eval("LocationCode") %></td>
