<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="UserInRoleEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanelHQ.Program.UserInRoleEntryCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    function getCheckedMember() {
        ...
        var lstSelectedIsMainRole = [];
        var result = '';
        $('#tblSelectedItem .trSelectedItem').each(function () {
            var key = $(this).find('.keyField').val();
            var isMainRole = $(this).find('.chkIsMainRole input').is(':checked') ? '1' : '0';
            lstSelectedMember.push(key);
            lstSelectedIsMainRole.push(isMainRole);
        });
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{RoleName}/g, $selectedTr.find('.tdRoleName').html());
            $newTr = $newTr.replace(/\$\{RoleID}/g, $selectedTr.find('.keyField').html());

            $newTr = $($newTr);
            if ($('#tblSelectedItem .trSelectedItem').length < 1)
                $newTr.find('.chkIsMainRole input').prop('checked', true);
            ...
            $('#tblSelectedItem .trSelectedItem').each(function () {
                if ($(this).find('.keyField').val() == id) {
                    $(this).remove();
                    if ($(this).find('.chkIsMainRole input').is(':checked')) {
                        if ($('#tblSelectedItem .trSelectedItem').length > 0)
                            $('#tblSelectedItem .trSelectedItem:eq(0)').find('.chkIsMainRole input').prop('checked', true);
                    }
                }
            });
        }
    ...
    $('.chkIsMainRole input').die('change');
    $('.chkIsMainRole input').live('change', function () {
        if ($(this).is(':checked')) {
            $('#tblSelectedItem .trSelectedItem .chkIsMainRole input:checked').each(function () {
    ...
    $('#tblSelectedItem .chkIsSelected2').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $tr = $(this).closest('tr');
            $tr.remove();
            if ($tr.find('.chkIsMainRole input').is(':checked')) {
                if ($('#tblSelectedItem .trSelectedItem').length > 0)
                    $('#tblSelectedItem .trSelectedItem:eq(0)').find('.chkIsMainRole input').prop('checked', true);
            }
        }
    ...
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${RoleID}' />
            </td>
            <td>${RoleName}</td>
            <td align="center"><asp:CheckBox ID="chkIsMainRole" CssClass="chkIsMainRole" runat="server"/></td>
        </tr>
    </script>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="RoleName" HeaderText="Nama" ItemStyle-CssClass="tdRoleName" />
                                    </Columns>
                                    <EmptyDataTemplate>
                ...
                <fieldset id="fsTrxPopup">
                    <table id="tblSelectedItem" class="grdView notAllowSelect" cellspacing="0" rules="all" >
                        <tr id="trHeader2">
                            ...
                            <th style="width:80px" class="thCenter"><%=GetLabel("Main Role")%></th> 
                        </tr>
                        <asp:Repeater ID="rptSelected" runat="server">
                            <ItemTemplate>
                                <tr class="trSelectedItem">
                                    <td align="center">
                                        <input type="checkbox" class="chkIsSelected2" />
                                        <input type="hidden" class="keyField" value='<%#Eval("RoleID") %>' />
                                    </td>
                                    <td><%#Eval("RoleName") %></td>
                                    <td align="center"><asp:CheckBox ID="chkIsMainRole" CssClass="chkIsMainRole" runat="server" Checked='<%#Eval("IsMainRole") %>' /></td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
                        <tr id="trFooter"></tr>
