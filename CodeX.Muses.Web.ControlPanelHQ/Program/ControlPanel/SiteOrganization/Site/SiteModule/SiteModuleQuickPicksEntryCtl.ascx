<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SiteModuleQuickPicksEntryCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.ControlPanelHQ.Program.SiteModuleQuickPicksEntryCtl" %>
...
<script type="text/javascript" id="dxss_testorderquickpicksctl">
    ...
    $('#<%=grdView.ClientID %> .chkIsSelected input').live('change', function () {
        if ($(this).is(':checked')) {
            ...
            $newTr = $newTr.replace(/\$\{ItemName}/g, $selectedTr.find('.tdItemName').html());
            $newTr = $newTr.replace(/\$\{ModuleID}/g, $selectedTr.find('.keyField').html());
            $newTr = $($newTr);
    ...
    <script id="tmplSelectedTestItem" type="text/x-jquery-tmpl">
        <tr class="trSelectedItem">
            <td align="center">
                ...
                <input type="hidden" class="keyField" value='${ModuleID}' />
            </td>
            <td>${ItemName}</td>
        </tr>
    </script>
                            ...
                            <asp:Panel runat="server" ID="pnlView" Style="width: 100%; margin-left: auto; margin-right: auto; position: relative;font-size:0.95em;">
                                ...
                                OnRowDataBound="grdView_RowDataBound">
                                    <Columns>
                                        ...
                                        <asp:BoundField DataField="ModuleName" HeaderText="Nama" ItemStyle-CssClass="tdItemName" />
                                    </Columns>
                                    <EmptyDataTemplate>
