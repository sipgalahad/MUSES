function QISClientQuickEntry() {
    ...
    this.SetFilterExpression = function (idx, filterExpression) {
        _self.columns[idx].filterExpression = filterExpression;
    }
...
function QISClientQuickEntryHelper() {
    var _self = this;
    this.timer = null;
    ...
    this.setTextBold = function () {
        ...
        var intellisenseText = '';
        var intellisenseDescription = '';
        for (var i = 0; i < _self.clientInstance.columns.length; ++i) {
            if (intellisenseText != '')
                intellisenseText += ';';

            if (i == numSeparator) {
                intellisenseText += '<b>' + _self.clientInstance.columns[i].text + '</b>';
                if (_self.clientInstance.columns[i].description != '')
                    intellisenseDescription = '<b>' + _self.clientInstance.columns[i].text + '</b> : ' + _self.clientInstance.columns[i].description;
            }
            else
                intellisenseText += _self.clientInstance.columns[i].text;
        }
        $('#' + _self.clientID + '_divAutoComplete .autoCompleteIntellisenseContentText').html(intellisenseText);
        $('#' + _self.clientID + '_divAutoComplete .intellisenseDescription').html(intellisenseDescription);
    }
    this.initializeControl = function () {
        ...
        $("#" + _self.clientID + "_divAutoComplete .txtAutoComplete").keydown(function (e) {
            ...
            else if (code == 59 || code == 186 || code == 9) { // 59 => ;     9 => Tab
                var textValue = $('#' + _self.clientID + '_divAutoComplete .txtAutoComplete').val();
                var paramTextValue = textValue.split(';');
                textValue = textValue.substring(0, _self.doGetCaretPosition());
                var numSeparator = (textValue.split(";").length - 1);

                if (_self.clientInstance.columns[numSeparator].methodName != null && _self.clientInstance.columns[numSeparator].methodName != '') {
                    e.preventDefault();
                    //if (_self.idxSelectedRow > 0) {
                    $row = $('#' + _self.clientID + '_divAutoComplete .tblAutoCompleteContent tr[class=selected]');
                    _self.selectRow($row[0]);
        ...
        $('#' + _self.clientID + '_divAutoComplete .tblAutoCompleteContent').find("tr:gt(0)").live("click", function () {
            ...
            /*$("#" + _self.clientID + "_divAutoComplete .autoCompleteIntellisenseBox").fadeIn('fast', function () {
            $("#" + _self.clientID + "_divAutoComplete .containerAutoCompleteIntellisenseBox").show();
            })*/
        });
    ...
    this.getAutoComplete = function () {
        ...
        if (_self.clientInstance.columns[numSeparator].methodName != null && _self.clientInstance.columns[numSeparator].methodName != '') {
            ...
            var filterExpression = _self.clientInstance.columns[numSeparator].filterExpression;
            if (filterExpression != '')
                filterExpression += ' AND ';
            if (_self.clientInstance.columns[numSeparator].column != null) {
                var col = _self.clientInstance.columns[numSeparator].column;
                var filter = '';
                ...
                filterExpression += _self.clientInstance.columns[numSeparator].fieldName + " LIKE '%" + paramTextValue[numSeparator] + "%'";
            _self.getListObject(_self.clientInstance.columns[numSeparator].methodName, filterExpression);
            // }
            // else {
            //    $('#' + _self.clientID + '_divAutoComplete .autoCompleteBox').fadeOut();
            //}
    ...
    this.selectRow = function (row) {
        if (typeof row == 'undefined') {
            ...
            if (_self.clientInstance.columns[numSeparator].methodName != null && _self.clientInstance.columns[numSeparator].methodName != '') {
                ...
                _self.getObject(_self.clientInstance.columns[numSeparator].methodName, filterExpression, function (entity) {
                    if (entity != null) {
                        if (typeof entity[_self.clientInstance.columns[numSeparator].fieldName] != 'undefined')
                            paramTextValue[numSeparator] = entity[_self.clientInstance.columns[numSeparator].fieldName];
                        ...
                        var paramValue = value.split(';');
                        if (typeof entity[_self.clientInstance.columns[numSeparator].fieldName] != 'undefined')
                            paramValue[numSeparator] = entity[_self.clientInstance.columns[numSeparator].valueField];
                        else
                            paramValue[numSeparator] = paramTextValue[numSeparator];
