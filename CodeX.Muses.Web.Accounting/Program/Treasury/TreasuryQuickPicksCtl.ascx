<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TreasuryQuickPicksCtl.ascx.cs" 
    Inherits="CodeX.Muses.Web.Accounting.Program.TreasuryQuickPicksCtl" %>
...
<script type="text/javascript" id="dxss_drugslogisticsquickpicksctl">
    ...
    $('#<%=btnCommit.ClientID %>').click(function () {
        if (tacQPCOA.getValue() == '')
            ...
            getCheckedMember();
            fillTransactionDt(tacQPCOA.getValue(), $('#<%=hdnSelectedMember.ClientID %>').val(), $('#<%=hdnSelectedMemberName.ClientID %>').val(), $('#<%=hdnSelectedMemberQty.ClientID %>').val(), $('#<%=txtRemarks.ClientID %>').val());
